import issues_fs_dev_role_librarian
...
class test_Version(TestCase):
    ...
    def test_path_code_root(self):
        assert self.version.path_code_root() == issues_fs_dev_role_librarian.path

    def test_path_version_file(self):
        with self.version as _:
            assert parent_folder(_.path_version_file()) == issues_fs_dev_role_librarian.path
            assert file_name    (_.path_version_file()) == 'version'

    def test_value(self):
        assert self.version.value() == version__issues_fs_dev_role_librarian
